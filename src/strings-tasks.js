...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < wordsArray.length; i += 1) {
    if (longestWordLenght < wordsArray[i].length) {
...
function invertCase(str) {
  ...
  const invertsChars = chars.map((char) => {
    let newChar;
...
function encodeToRot13(str) {
  ...
  const encodeStr = splitStr.map((char) => {
    if (/[a-zA-Z]/.test(char)) {
      const base = char.charCodeAt(0) < 97 ? 65 : 97;
      const cipher = (char.charCodeAt(0) - base + 13) % 26;
      return String.fromCharCode(base + cipher);
    }
    return char;
  });
